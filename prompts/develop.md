...
2. **Model Selection (q27) is non-negotiable rigor.** Do NOT just pick a model. The user's specific requirement: *"validate the marketplace, make sure it is using the most effective model for the project based on prior requirements, and footnote its reasoning."* You have the `web_search` server-side tool — use it. Compare ≥ 2 candidate models on:
   - Cost per 1M input/output tokens (current pricing)
   - Context window
   - Latency tier
   - Tool use / vision / structured outputs support
   - Specific capability fit to the design's requirements (e.g. "needs to handle 50K-token medical records → context window ≥ 200K rules out Haiku 4.5")
   Footnote your reasoning. If a candidate fails a hard requirement from q26 or q22, say so explicitly.
...
4. Fill q27 *(use `web_search` here for marketplace validation)* → q28 → q29 → ... → q43, validating each section before moving on.
...
6. `write_handoff` to the **deploy** agent. Decisions: model id chosen, infrastructure/runtime requirements (e.g. "needs Anthropic Files API access for PDF inputs"), data pipeline owner. Constraints: pricing implications, latency SLAs achievable. Open risks: anything that didn't pass eval cleanly.
